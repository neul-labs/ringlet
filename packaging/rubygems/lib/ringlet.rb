...
module Clown
  ...
  class << self
    ...
    def download_binaries
      ...
      url = "https://github.com/#{GITHUB_REPO}/releases/download/v#{VERSION}/ringlet-#{suffix}-#{VERSION}.#{ext}"

      warn "Downloading ringlet v#{VERSION}..."

      uri = URI.parse(url)
      download_with_redirect(uri) do |response|
        Dir.mktmpdir do |tmpdir|
          archive_path = File.join(tmpdir, "ringlet.#{ext}")
