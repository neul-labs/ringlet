...
pub async fn start_daemon(state: State<'_, AppState>) -> Result<String, AppError> {
    ...
    // Use current executable with "daemon" subcommand
    let ringlet_path = std::env::current_exe().map_err(|e| {
        AppError::Daemon(format!("Cannot determine current executable path: {}", e))
    })?;

    // Spawn the daemon
    let child = Command::new(&ringlet_path)
        .args(["daemon", "--stay-alive", "--foreground"])
