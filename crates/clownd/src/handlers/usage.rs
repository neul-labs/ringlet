...
fn merge_agent_scan_data(
    ...
    use clown_core::usage::ModelUsage;
    ...
    for entry in &scan.entries {
        ...
        if let Some(cost_usd) = entry.cost_usd {
            if let Some(ref mut cost) = model_usage.cost {
                ...
                model_usage.cost = Some(clown_core::CostBreakdown {
                    total_cost: cost_usd,
                    ..Default::default()
                });
            }
    ...
    for entry in &scan.entries {
        ...
        if let Some(cost_usd) = entry.cost_usd {
            if let Some(ref mut total_cost) = aggregates.total_cost {
                ...
                aggregates.total_cost = Some(clown_core::CostBreakdown {
                    total_cost: cost_usd,
                    ..Default::default()
                });
            }
...
) -> (TokenUsage, Option<clown_core::CostBreakdown>) {
...
fn convert_to_usage_aggregates(
    ...
    use clown_core::usage::ProfileUsage;
    use std::collections::HashMap;
