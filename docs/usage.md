...
The usage tracking feature provides native token and cost monitoring for all agent sessions, similar to tools like ccusage. It tracks token consumption across all profiles and calculates costs for profiles using direct API keys.
...
- **Cost Calculation**: Estimated costs based on LiteLLM's pricing database (only for "self" provider)
...
- [Providers](providers.md) - Provider configuration including "self" provider
- [Registry](registry.md) - Registry sync including pricing data
