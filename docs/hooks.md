...
Hooks are profile-level configurations that trigger actions when specific events occur during agent execution. Each profile can have its own set of hooks, providing fine-grained control over different workflows.
...
- The agent must support hooks (`supports_hooks: true` in manifest)
- Currently, **Claude Code** is the primary agent with hooks support
...
Hooks are stored in JSON format with the following structure:
...
Each event type contains an array of rules:
...
- [Scripting](scripting.md) - Rhai configuration scripts
