...
Providers define API backends that profiles connect to. They specify endpoints, authentication requirements, and available models. By separating providers from agents, clown lets you run the same CLI tool (e.g., Claude Code) against different backends (Anthropic, MiniMax, OpenRouter) without modifying agent manifests.
...
```toml
# ~/.config/clown/providers.d/minimax.toml
id = "minimax"
name = "MiniMax"
...
When you create a profile, clown combines information from the agent manifest, provider manifest, and user preferences:
...
1. Create a TOML file in `~/.config/clown/providers.d/`:
...
```bash
$ clown profiles create claude work-internal --provider my-gateway
```
...
Instead of following provider-specific setup guides (editing `~/.claude/settings.json`, `.codex/config.toml`, etc.), use clown:

| Manual approach | clown approach |
|-----------------|-------------------|
| Edit `~/.claude/settings.json` with MiniMax env vars | `clown profiles create claude work --provider minimax` |
| Edit `.codex/config.toml` with provider block | `clown profiles create codex work --provider minimax` |
| Edit `~/.factory/config.json` with custom_models | `clown profiles create droid work --provider minimax` |
| Export `GROK_BASE_URL` and `GROK_API_KEY` | `clown profiles create grok work --provider minimax` |

clown handles the configuration details per agent and provider, letting you focus on which tool and which backend you want to use.
